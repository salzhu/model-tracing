...
model_ft = AutoModelForCausalLM.from_pretrained("lmsys/vicuna-7b-v1.5", torch_dtype=torch.bfloat16)
...
target_ft['model.embed_tokens.weight'] = target_ft['model.embed_tokens.weight'][:,permutation]
...
for i in range(32):
    ...
    target_ft['model.layers.'+str(i)+'.mlp.gate_proj.weight'] = target_ft['model.layers.'+str(i)+'.mlp.gate_proj.weight'][:,permutation] # fixed
    target_ft['model.layers.'+str(i)+'.mlp.up_proj.weight'] = target_ft['model.layers.'+str(i)+'.mlp.up_proj.weight'][:,permutation] # fixed
    target_ft['model.layers.'+str(i)+'.mlp.down_proj.weight'] = target_ft['model.layers.'+str(i)+'.mlp.down_proj.weight'][permutation] # fixed
...
print("post_attention_layernorm" + str(target['model.layers.0.post_attention_layernorm.weight'].shape))
