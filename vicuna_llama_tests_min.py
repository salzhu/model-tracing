...
model_llama = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf", torch_dtype=torch.bfloat16).to('cuda')
...
model_temp = AutoModelForCausalLM.from_pretrained("lmsys/vicuna-7b-v1.5", torch_dtype=torch.bfloat16).to('cuda')
...
for step in steps: 
    ...
    with torch.no_grad():
        ...
        target_temp['model.embed_tokens.weight'][:][:] = step * target_vicuna['model.embed_tokens.weight'][:][:] + (1 - step) * target_llama['model.embed_tokens.weight'][:32000][:]

        for i in range(32):
            for layer in layer_2d_names:
                layer_name = 'model.layers.' + str(i) + layer
                target_temp[layer_name][:][:] = step * target_vicuna[layer_name][:][:] + (1 - step) * target_llama[layer_name][:][:]
            
            for layer in layer_1d_names:
                layer_name = 'model.layers.' + str(i) + layer
                target_temp[layer_name][:] = step * target_vicuna[layer_name][:] + (1 - step) * target_llama[layer_name][:]
        
        target_temp['model.norm.weight'][:] = step * target_vicuna['model.norm.weight'][:] + (1 - step) * target_llama['model.norm.weight'][:]
        target_temp['lm_head.weight'][:][:] = step * target_vicuna['lm_head.weight'][:][:] + (1 - step) * target_llama['lm_head.weight'][:32000][:]
        ...
        for item in test_dataloader:
            ...
            logits = model_temp(input_ids = inputs["input_ids"], labels = inputs["input_ids"]).logits
