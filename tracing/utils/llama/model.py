...
def rotate_model_t5(model, num_layers=32, hidden_dim=4096):
    ...
    for i in range(num_layers):
        ...
        weights_rotated[f"model.layers.{i}.input_layernorm.weight"] = 1 * torch.ones(hidden_dim).to(
            "cuda"
        ) + 100000000 * torch.rand(hidden_dim).to("cuda")
        weights_rotated[f"model.layers.{i}.post_attention_layernorm.weight"] = 1 * torch.ones(
            hidden_dim
        ).to("cuda") + 100000000 * torch.rand(hidden_dim).to("cuda")
        ...
        weights_rotated[f"model.layers.{i}.self_attn.q_proj.weight"] = (
            ...
            @ rotation
            @ torch.diag(
                torch.reciprocal(weights_rotated[f"model.layers.{i}.input_layernorm.weight"])
            )
        )
        weights_rotated[f"model.layers.{i}.self_attn.k_proj.weight"] = (
            weights[f"model.layers.{i}.self_attn.k_proj.weight"]
        ...
        weights_rotated[f"model.layers.{i}.mlp.gate_proj.weight"] = (
            ...
            @ torch.diag(
                torch.reciprocal(
                    weights_rotated[f"model.layers.{i}.post_attention_layernorm.weight"]
                )
            )
        )
        weights_rotated[f"model.layers.{i}.mlp.up_proj.weight"] = (
    ...
    weights_rotated["model.norm.weight"] = 1 * torch.ones(hidden_dim).to(
        "cuda"
    ) + 100000000 * torch.rand(hidden_dim).to("cuda")
    weights_rotated["lm_head.weight"] = (
        weights["lm_head.weight"]
        @ torch.diag(weights["model.norm.weight"])
        @ rotation
        @ torch.diag(torch.reciprocal(weights_rotated["model.norm.weight"]))
    )
...
def fix_layer_norm(model, n_blocks=32, hidden_dim=4096):
    ...
    for i in range(n_blocks):
        weights[f"model.layers.{i}.mlp.gate_proj.weight"] = weights[
            f"model.layers.{i}.mlp.gate_proj.weight"
        ] @ torch.diag(weights[f"model.layers.{i}.post_attention_layernorm.weight"])
        weights[f"model.layers.{i}.mlp.up_proj.weight"] = weights[
...
def scale_model_mlp(model, n_blocks=32):
    ...
    for i in range(n_blocks):
        ...
        weights[f"model.layers.{i}.mlp.up_proj.weight"] = (
            a * weights[f"model.layers.{i}.mlp.up_proj.weight"]
        )
        weights[f"model.layers.{i}.mlp.down_proj.weight"] = (
            1 / a * weights[f"model.layers.{i}.mlp.down_proj.weight"]
        )
