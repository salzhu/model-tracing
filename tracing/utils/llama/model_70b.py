...
def avg_mlp_block(model0, model1, i, alpha=0.5):
    model1_weight = model1.state_dict()['model.layers.' + str(i) + '.mlp.gate_proj.weight'].to(model0.device)
    model0.state_dict()['model.layers.' + str(i) + '.mlp.gate_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.mlp.gate_proj.weight'] + (1 - alpha) * model1_weight
    del model1_weight

    model1_weight = model1.state_dict()['model.layers.' + str(i) + '.mlp.up_proj.weight'].to(model0.device)
    model0.state_dict()['model.layers.' + str(i) + '.mlp.up_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.mlp.up_proj.weight'] + (1 - alpha) * model1_weight
    del model1_weight

    model1_weight = model1.state_dict()['model.layers.' + str(i) + '.mlp.down_proj.weight'].to(model0.device)
    model0.state_dict()['model.layers.' + str(i) + '.mlp.down_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.mlp.down_proj.weight'] + (1 - alpha) * model1_weight
    del model1_weight

def avg_transformer_block(model0, model1, i, alpha=0.5, attn=True):
    if attn:
        model1_weight = model1.state_dict()['model.layers.' + str(i) + '.self_attn.q_proj.weight'].to(model0.device)
        model0.state_dict()['model.layers.' + str(i) + '.self_attn.q_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.self_attn.q_proj.weight'] + (1 - alpha) * model1_weight
        del model1_weight

        model1_weight = model1.state_dict()['model.layers.' + str(i) + '.self_attn.k_proj.weight'].to(model0.device)
        model0.state_dict()['model.layers.' + str(i) + '.self_attn.k_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.self_attn.k_proj.weight'] + (1 - alpha) * model1_weight
        del model1_weight

        model1_weight = model1.state_dict()['model.layers.' + str(i) + '.self_attn.v_proj.weight'].to(model0.device)
        model0.state_dict()['model.layers.' + str(i) + '.self_attn.v_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.self_attn.v_proj.weight'] + (1 - alpha) * model1_weight
        del model1_weight

        model1_weight = model1.state_dict()['model.layers.' + str(i) + '.self_attn.o_proj.weight'].to(model0.device)
        model0.state_dict()['model.layers.' + str(i) + '.self_attn.o_proj.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.self_attn.o_proj.weight'] + (1 - alpha) * model1_weight
        del model1_weight

    avg_mlp_block(model0, model1, i, alpha=alpha)

    model1_weight = model1.state_dict()['model.layers.' + str(i) + '.input_layernorm.weight'].to(model0.device)
    model0.state_dict()['model.layers.' + str(i) + '.input_layernorm.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.input_layernorm.weight'] + (1 - alpha) * model1_weight
    del model1_weight

    model1_weight = model1.state_dict()['model.layers.' + str(i) + '.post_attention_layernorm.weight'].to(model0.device) 
    model0.state_dict()['model.layers.' + str(i) + '.post_attention_layernorm.weight'] = alpha * model0.state_dict()['model.layers.' + str(i) + '.post_attention_layernorm.weight'] + (1 - alpha) * model1_weight
    del model1_weight

def avg_embedding_layer(model0, model1, alpha=0.5):
    model1_weight = model1.state_dict()['model.embed_tokens.weight'].to(model0.device)
    model0.state_dict()['model.embed_tokens.weight'] = alpha * model0.state_dict()['model.embed_tokens.weight'] + (1 - alpha) * model1_weight
    del model1_weight

def avg_output_layer(model0, model1, alpha=0.5):
    model1_weight = model1.state_dict()['lm_head.weight'].to(model0.device)
    model0.state_dict()['lm_head.weight'] = alpha * model0.state_dict()['lm_head.weight'] + (1 - alpha) * model1_weight
    del model1_weight

    model1_weight = model1.state_dict()['model.norm.weight'].to(model0.device)
    model0.state_dict()['model.norm.weight'] = alpha * model0.state_dict()['model.norm.weight'] + (1 - alpha) * model1_weight
    del model1_weight

def avg_model(avg_model, model1, alpha=0.5, n_blocks=80, attn=True, emb=True):
    if emb:
