...
from tracing.statistics.mlp_gate_up_matching import statistic, rotate_model, mlp_matching_gate, mlp_matching_up
...
def main():
    ...
    # model_retrained = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch.bfloat16).to("cuda")
    # model_retrained.config.intermediate_size = config.intermediate_size

    config_retrained = LlamaConfig(intermediate_size=config.intermediate_size)
    ...
    # print("csw spearman p-value: " + str(csw(model, model_retrained)[0]))

    # print("Rotate and permute...")

    # rotate_model(model_retrained)
    # permute_model(model_retrained, model_retrained, torch.randperm(11008), torch.randperm(4096))

    # rotate_model(model)
    # permute_model(model, model, torch.randperm(11008), torch.randperm(4096))

    # print("csw spearman p-value: " + str(csw(model, model_retrained)[0]))

    print("llama2 and vicuna1.5")
