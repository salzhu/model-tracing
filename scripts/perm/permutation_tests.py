...
def permuted_mode_connectivity_l2(model_base, model_ft, tokenizer, alpha_step=0.1, end_points=False):
    ...
    for alpha in alphas:
        ...
        # L2 calculation
        temp, a, b = calculate_l2_distance(model_base, model_ft)
        l2s.append(temp)
        ...
        for batch in tqdm(eval_dataloader):
            ...
            # compute_jsd_tokenized(model_base, model_ft, batch.to(device))
        ...
        del interpolated_model, input_ids, attention_mask, labels, outputs, loss
        torch.cuda.empty_cache()
        print("alpha = " + str(alpha) + " | " + str(loss_mean) + " | " + str(l2s[0]))
        # print(f"One interpolation eval time: {time.time()-time2} seconds")

    print(f"Total run time: {time.time()-time0} seconds")
    return losses, l2s

def permuted_l2(model_base, model_ft, tokenizer, alpha_step=0.1, end_points=False):
...
def save_permuted_mode_connectivity_loss(csv_filename, model_a_name, model_b_name, alphas, losses):

    csv_header = ["Model Pair", "time"] + [f"Alpha {alpha} loss" for alpha in alphas]
    ...
    with open(csv_filename, "a", newline="") as csvfile:
        ...
        row = [model_pair, datetime.datetime.now()] + losses
        writer.writerow(row)
