...
from tracing.utils.llama.model import rotate_model, fix_layer_norm, rotate_model_t5
...
model_rotated = AutoModelForCausalLM.from_pretrained(model_rot_name, torch_dtype=torch.bfloat16).to('cuda')
...
model.to('cuda')
print("invariant 1")
invariant = weights['model.embed_tokens.weight']@weights['model.layers.0.self_attn.q_proj.weight'].T@weights['model.layers.0.self_attn.k_proj.weight']@(weights['model.embed_tokens.weight']).T
model.to('cpu')
model_rotated.to('cuda')
invariant_rotated = weights_rotated['model.embed_tokens.weight']@weights_rotated['model.layers.0.self_attn.q_proj.weight'].T@weights_rotated['model.layers.0.self_attn.k_proj.weight']@(weights_rotated['model.embed_tokens.weight']).T
model_rotated.to('cpu')
invariant.to('cuda')
invariant_rotated.to('cuda')
invariant = torch.flatten(invariant)
invariant_rotated = torch.flatten(invariant_rotated)
print(torch.dot(invariant, invariant_rotated) / (torch.norm(invariant) * torch.norm(invariant_rotated)))

model.to('cuda')
print("invariant 2")
invariant = weights['model.embed_tokens.weight']@weights['model.layers.0.self_attn.v_proj.weight'].T@weights['model.layers.0.self_attn.o_proj.weight'].T@weights['model.embed_tokens.weight'].T
model.to('cpu')
model_rotated.to('cuda')
invariant_rotated = weights_rotated['model.embed_tokens.weight']@weights_rotated['model.layers.0.self_attn.v_proj.weight'].T@weights_rotated['model.layers.0.self_attn.o_proj.weight'].T@weights_rotated['model.embed_tokens.weight'].T
model_rotated.to('cpu')
invariant.to('cuda')
invariant_rotated.to('cuda')
invariant = torch.flatten(invariant)
...
model.to('cuda')
print("invariant 3")
invariant = weights['model.embed_tokens.weight']@weights[f'model.layers.0.mlp.up_proj.weight'].T@weights[f'model.layers.0.mlp.down_proj.weight'].T@weights['model.embed_tokens.weight'].T
model.to('cpu')
model_rotated.to('cuda')
invariant_rotated = weights_rotated['model.embed_tokens.weight']@weights_rotated[f'model.layers.0.mlp.up_proj.weight'].T@weights_rotated[f'model.layers.0.mlp.down_proj.weight'].T@weights_rotated['model.embed_tokens.weight'].T
model_rotated.to('cpu')
invariant.to('cuda')
invariant_rotated.to('cuda')
invariant = torch.flatten(invariant)
